...
class DockerPorts():
    ...
    def readConfig(self, fn):
        ...
        if "global" in config.sections() and "logfile" in config["global"]:
            ...
            elif config["global"]["logfile"] == "STDERR":
    ...
    def create(self, profilename):
        ...
        if imagelimit > 0 and icount >= imagelimit:
            logger.warning("Reached max count of {} (currently {}) for image {}".format(imagelimit, icount, profilename))
            return None
...
class DockerInstance():
    ...
    def getMappedPort(self, inp):
        try:
            ...
            logger.warning("Failed to get port information for port {} from {}: {}".format(inp, self.getInstanceID(), e))
        return None
    ...
    def getInstanceID(self):
        try:
            ...
            logger.warning("Failed to get instanceid: {}".format(e))
        return "None"
    ...
    def stop(self):
        ...
        try:
            ...
            logger.warning("Failed to remove instance for middleport {}, id {}".format(mp, cid))
            return False
        return True
...
def get_config_file(argv):
    ...
    with tempfile.NamedTemporaryFile("w+t", delete=False) as tf:
        with open("/etc/ssh-switchboard/global.conf", "rt") as gl:
            tf.write(gl.read())
        print(file=tf)
        for node, dirs, files in os.walk("/etc/ssh-switchboard/conf.d"):
            for f in files:
                name = os.path.join(node, f)
                logger.debug("appending config file '{}'...".format(name))
...
if __name__ == "__main__":
    ...
    portsAndNames = globalDockerPorts.readConfig(get_config_file(sys.argv))
